...
| M8 协议兼容与更多驱动 | 🔴 进行中（当前前沿）| 待开发（接入 AWS S3 与百度网盘）|

> README 的"开发阶段"表已随 M7 更新至 M0–M7 ✅、M8 计划中。
...
M0–M7 已完成（见上方"进度对账"）。**下一阶段为 M8 协议兼容与更多驱动**：接入 AWS S3（读写、预签名直链重定向）与百度网盘（读取/下载、代理流），并在 `web/` 增强存储管理页的多驱动表单。
