...
- **限制（重要）**：约定**一个物理目录只配一个挂载点**。若两个 storage 的 rootPath 指向同一物理目录，同一文件会被索引两份（按各自 storage_id）；从其中一个挂载删除只发该 storage 的删除事件，另一挂载的索引行会成为孤儿（物理文件已删、搜索仍命中），需对另一存储重建索引才清除。增量删除按"操作所在 storage"定位，不扫描其他 storage 的重叠根目录（避免为畸形配置打补丁）。
